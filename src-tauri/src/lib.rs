...
async fn quick_scan(folder_path: String) -> Result<QuickScanResult, String> {
    ...
    let folders: Vec<FolderInfo> = folder_map.into_iter()
        .map(|(path, count)| {
...
fn generate_fast_thumbnail(source_path: &Path, app_handle: &AppHandle, image_id: &str) -> Result<String, String> {
...
async fn copy_to_library(
    ...
    fs::copy(source, &dest_path)
        .map_err(|e| format!("Failed to copy to library: {}", e))?;
